...
module FsToolkit.ErrorHandling.Result
...
let inline teeIf
    ...
    : Result<'ok, 'error> =
    match result with
    | Ok x when predicate x -> sideEffect x
    | _ -> ()
...
let inline teeErrorIf
    ...
    : Result<'ok, 'error> =
    match result with
    | Error x when predicate x -> sideEffect x
    | _ -> ()
...
[<System.Obsolete "Please use defaultWith instead of valueOr">]
...
let req reqF errOrF value = bind (reqF errOrF) value
let reqFilter predicate errOrF value = req (Req.filter predicate) errOrF value

/// <summary>
/// When <paramref name="result"/> is <c>Ok</c>, applies <paramref name="checker"/> to the <c>Ok</c> value.
/// If <paramref name="checker"/> returns an <c>Ok</c> unit value, returns <paramref name="result"/>.
/// Otherwise, returns the <c>Error</c> value from the <paramref name="checker"/> as a new result.
///
...
/// </summary>
/// <param name="checker">The function that performs a check against the <c>Ok</c> value of <paramref name="result"/>. Returns <c>Error</c> value from the function if error.</param>
/// <param name="result">The input result.</param>
/// <returns>The input <paramref name="result"/> if <c>Error</c> or <paramref name="checker"/> returns a <c>Ok</c> result. Returns the <c>Error</c> value from <paramref name="checker"/> if it returns an <c>Error</c> result.</returns>
let inline check
    ([<InlineIfLambda>] checker: 'ok -> Result<unit, 'error>)
    (result: Result<'ok, 'error>)
    : Result<'ok, 'error> =
    req (Req.check checker) () result

[<System.Obsolete "Please use reqFilter predicate error instead of require predicate error">]
let inline require predicate error result = reqFilter predicate error result

[<System.Obsolete "Please use (x1, x2) |> Req.equal error instead of requireEqual x1 x2 error">]
let inline requireEqual x1 x2 error =
    (x1, x2)
    |> Req.equal error

[<System.Obsolete "Please use Req.isTrue error instead of requireTrue error">]
let inline requireTrue error value = Req.isTrue error value

[<System.Obsolete "Please use Req.isTrueWith errorF instead of requireTrueWith errorF">]
let requireTrueWith errorF value = Req.isTrueWith errorF value

[<System.Obsolete "Please use Req.isFalse error instead of requireFalse error">]
let inline requireFalse error value = Req.isFalse error value

[<System.Obsolete "Please use Req.isFalseWith errorF instead of requireFalseWith errorF">]
let requireFalseWith errorF value = Req.isFalseWith errorF value

[<System.Obsolete "Please use Req.some error instead of requireSome error">]
let inline requireSome error value = Req.some error value

[<System.Obsolete "Please use Req.someWith errorF instead of requireSomeWith errorF">]
let requireSomeWith errorF value = Req.someWith errorF value

[<System.Obsolete "Please use Req.none error instead of requireNone error">]
let inline requireNone error value = Req.none error value

[<System.Obsolete "Please use Req.noneWith errorF instead of requireNoneWith errorF">]
let requireNoneWith errorF value = Req.noneWith errorF value

[<System.Obsolete "Please use Req.valueSome error instead of requireValueSome error">]
let inline requireValueSome error value = Req.valueSome error value

[<System.Obsolete "Please use Req.valueSomeWith errorF instead of requireValueSomeWith errorF">]
let requireValueSomeWith errorF value = Req.valueSomeWith errorF value

[<System.Obsolete "Please use Req.valueNone error instead of requireValueNone error">]
let inline requireValueNone error value = Req.valueNone error value

[<System.Obsolete "Please use Req.valueNoneWith errorF instead of requireValueNoneWith errorF">]
let requireValueNoneWith errorF value = Req.valueNoneWith errorF value

[<System.Obsolete "Please use Req.notNull error instead of requireNotNull error">]
let inline requireNotNull error value = Req.notNull error value

[<System.Obsolete "Please use Req.equalTo other error instead of requireEqualTo other error">]
let inline requireEqualTo other error value = Req.equalTo other error value

[<System.Obsolete "Please use Req.empty error instead of requireEmpty error">]
let inline requireEmpty error value = Req.empty error value

[<System.Obsolete "Please use Req.notEmpty error instead of requireNotEmpty error">]
let inline requireNotEmpty error value = Req.notEmpty error value

[<System.Obsolete "Please use Req.head error instead of requireHead error">]
let inline requireHead error value = Req.head error value
